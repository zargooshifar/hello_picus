...
install_opencv () {
  ...
  if [ -e "/proc/device-tree/model" ]; then
	  ...
	  # Use always "-j 4"
          ...
          ARCH=5.3
          PTX="sm_53"
      elif [[ $model == *"Picus Board V1.1"* ]]; then
          echo "Detecting Picus Board V1.1"
          NO_JOB=8
          ARCH=8.7
          PTX="sm_87"
	  ...
	  if [[ "FREE_MEM" -gt "5500" ]]; then
	    ...
	    NO_JOB=1
          ...
          echo "Unable to determine the Picus model."
          exit 1
      fi
      echo ""
  else
      echo "Error: /proc/device-tree/model not found. Are you sure this is a Picus?"
      exit 1
  fi
  
  echo "Installing latest OpenCV from github on your Picus"
  echo "It will take ~1 hours!"
  ...
  echo "You've successfully installed latest OpenCV on your Picus"
}
